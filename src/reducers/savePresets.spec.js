import { addSavePreset, initSavePresets, removeSavePreset, setPresetId } from "../actions/savePresets";
...
describe("savePresets reducer", () => {
  ...
  it("should return state after replacing its presetsCollection with the payload array", function () {
    ...
    expect(savePresetsReducer(undefined, initSavePresets(payload))).toEqual(mockState);
	});
  ...
  it("should return state after removing the object with an id that matches the payload from its presetsCollection array", function () {
    ...
    expect(savePresetsReducer(mockInitialState, removeSavePreset(payload))).toEqual(mockState);
	});
